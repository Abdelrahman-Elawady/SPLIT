...
const int IN4 = 13;
const int ENB = 26;
const int freq = 5000;    // PWM frequency for motor control
...
float readBatteryVoltage() 
{
  ...
  return pin_voltage * 6.01; //(1970+ 9870) / 1970 voltage divider ratio (using an op amp would be better for zero current consumption)
}
...
void setup()
{
  ...
  Wire.setTimeOut(10); 
  //something to note about MPU6050: to avoid any freezing with it the only way to do it is via hardware (grounding should be perfect and power too)
  //prevent I2C bus lockup by setting a timeout for I2C transactions. If a transaction takes longer than 10 ms, it will be aborted and the bus will be released, allowing the robot to recover from potential communication issues with the MPU6050 sensor without freezing the entire system.
  mpu.initialize(); // Initialize the MPU6050 sensor

  // Manually set the accelerometer and gyroscope offsets via calibration script.
  //Your offsets:	-7023	3174	9117	-162	-246	29 this is different for every MPU6050 IMU
  //use the IMU calibration test module in the Testing folder to get these offsets and paste them here!
  mpu.setXAccelOffset(-7023); 
  mpu.setYAccelOffset(3174);
  mpu.setZAccelOffset(9117);
  mpu.setXGyroOffset(-162);
  mpu.setYGyroOffset(-246);
...
{ 
  ...
  if (abs(speed_left) < 45 && abs(speed_left) > 0) //use testing script of motors to figure it out! (optimize motor driver quality for better power for motors)
